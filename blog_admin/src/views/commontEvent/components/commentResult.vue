  ...
  <div class="app-container">
    <div v-show="mainTableIsShow" class="filter-container">
      <el-row type="flex" align="middle">
        ...
        >批量删除</el-button>
        <!-- <el-button v-waves :loading="downloadLoading" :disabled="multipleSelection.length == 0"  class="filter-item" type="primary" @click="handleDownload">
          批量下载
        </el-button>-->
        <span style="font-size:16px;margin-left:auto;">共{{ total }}篇文章，其中</span>
        <span style="color:red;font-size:16px;font-weight:700;">{{ negaitveTotal }}</span>
        <span style="font-size:16px;margin-right:30px;">
          篇文有负面评论
          <span v-if="total != 0">，占比</span>
          <span v-if="total != 0"
    ...
    >
      ...
      <el-table-column label="文章标题" min-width="150px" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="评论数量(负面数量)" align="center">
        <template slot-scope="{row}">
    ...
    <div v-if="!mainTableIsShow" class="filter-container">
      ...
      <el-row type="flex" align="top" style="margin-top:10px;">
        <el-input
          v-model="commentListQuery.author"
          placeholder="作者"
          style="width: 120px;margin-right:20px;"
          class="filter-item"
        />
        ...
        <div style="margin-right:3px;margin-left:3px;padding-top:10px;">
          <span>至</span>
        </div>
        <div>
          <el-date-picker
...
export default {
  ...
  data() {
    return {
      ...
      list: null, // 文章/视频的展示页面
      total: 0, // 舆情文章/视频总条数
      negaitveTotal: 0, // 负面舆情文章/视频条数
