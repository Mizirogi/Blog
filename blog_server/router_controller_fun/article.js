/*
 * @Description: 路由控制函数文章路由函数处理
 * @Author: Li
 * @Date: 2019-11-03 17:18:59
 * @LastEditors  : Li
 * @LastEditTime : 2020-01-13 20:00:51
 */
...
class Article {
     ...
     * @returns 文章列表数据
     */
    static async list_web(ctx) {
        let params = ctx.request.body;
        try {
            const data = await ArticleModel.list_web(params);
            ctx.response.status = 200;
            ctx.body = {
                code: 200,
                message: `查询文章列表成功`,
    ...
    static async detail(ctx) {
        ...
        try {
            let ret = await ArticleModel.detail(id);

            if (ret !== null) {
                // 浏览次数增加1
                let browser = ret.browser + 1;
                await ArticleModel.updateBrowser(id, browser);
            }


            let a = []
            for (let i = 0; i < ret.categories.length; i++) {
                a.push(ret.categories[i].id);
                
            }
            console.log(a);
            let data = ret.dataValues
            data.categories = a
            console.log(data);
            ctx.response.status = 200;
            ctx.body = {
    ...
    static async hidden(ctx) {
        ...
        try {
            await ArticleModel.hidden(id, { status });
            // let data = await ArticleModel.detail(id);
            ctx.response.status = 200;
            ctx.body = {
                code: 200,
